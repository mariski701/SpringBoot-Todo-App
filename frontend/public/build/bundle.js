...
var app = (function () {
    ...
    // (90:2) {#each filteredTodos as item}
    function create_each_block(ctx) {
    	let div1;
    	let div0;
    	let input;
    	let t_value = /*item*/ ctx[13].text + "";
    	let t;
    	let button;
    	let img;
    	let img_src_value;
    	...
    	function input_change_handler() {
    		/*input_change_handler*/ ctx[11].call(input, /*each_value*/ ctx[14], /*item_index*/ ctx[15]);
    	}
    	...
    	const block = {
    			...
    			if (!mounted) {
    				dispose = [
    					...
    					listen_dev(
    						...
    						"change",
    						function () {
    							if (is_function(/*doUpdate*/ ctx[3](/*item*/ ctx[13].id, /*item*/ ctx[13].done))) /*doUpdate*/ ctx[3](/*item*/ ctx[13].id, /*item*/ ctx[13].done).apply(this, arguments);
    						},
    						false,
    						false,
    						false
    					...
    					listen_dev(
    						...
    						"click",
    						function () {
    							if (is_function(/*doDelete*/ ctx[4](/*item*/ ctx[13].id))) /*doDelete*/ ctx[4](/*item*/ ctx[13].id).apply(this, arguments);
    						},
    						false,
    						false,
    						false
    		...
    		p: function update(new_ctx, dirty) {
    			...
    			if (dirty & /*filteredTodos*/ 2) {
    				input.checked = /*item*/ ctx[13].done;
    			}

    			if (dirty & /*filteredTodos*/ 2 && t_value !== (t_value = /*item*/ ctx[13].text + "")) set_data_dev(t, t_value);
    		},
    	...
    	dispatch_dev("SvelteRegisterBlock", {
    		...
    		type: "each",
    		source: "(90:2) {#each filteredTodos as item}",
    		ctx
    	});
    ...
    function create_fragment(ctx) {
    	...
    	const block = {
    		c: function create() {
    			...
    			add_location(a0, file, 78, 6, 1602);
    			...
    			attr_dev(div1, "class", "text-center col-sm-4 col-sm-offset-4");
    			add_location(div1, file, 75, 3, 1468);
    		...
    		m: function mount(target, anchor) {
    			...
    			if (!mounted) {
    				dispose = [
    					...
    					listen_dev(button, "click", /*doPost*/ ctx[2], false, false, false)
    				];
    		...
    		p: function update(ctx, [dirty]) {
    			if (dirty & /*doDelete, filteredTodos, doUpdate*/ 26) {
    ...
    function instance($$self, $$props, $$invalidate) {
    	...
    	onMount(async function () {
    		...
    		$$invalidate(6, data = await response.json());
    		console.log(data);
    	});
    	...
    	async function doUpdate(id, done) {
    		...
    		const response = await fetch(apiURL + "/updateTodo/" + id + "/" + done, {
    			"method": "PUT",
    			"headers": { "Content-Type": "application/json" },
    			"body": JSON.stringify({ _id: id, done })
    		});
    	...
    	async function doDelete(id) {
    		const res = await fetch(apiURL + "deleteTodo/" + id, { "method": "DELETE" });
    		const json = await res.json();
    		$$invalidate(6, data = json);
    	}

    	function updateFilter(filter) {
    		$$invalidate(7, currentFilter = filter);
    	}
    	...
    	function input_change_handler(each_value, item_index) {
    		each_value[item_index].done = this.checked;
    		(($$invalidate(1, filteredTodos), $$invalidate(7, currentFilter)), $$invalidate(6, data));
    	}
    	...
    	$$self.$inject_state = $$props => {
    		...
    		if ("currentFilter" in $$props) $$invalidate(7, currentFilter = $$props.currentFilter);
    	...
    	$$self.$$.update = () => {
    		if ($$self.$$.dirty & /*currentFilter, data*/ 192) {
